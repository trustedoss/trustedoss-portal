...
![비밀번호 찾기 페이지](./img/auth-forgot.png)
...
`/profile`은 본인을 로그인시킬 수 있는 모든 신원을 나열합니다.

- **Password** — 이메일+비밀번호로 가입했거나 이후 비밀번호를 설정한 경우 표시.
- **GitHub** — GitHub로 한 번이라도 로그인한 적이 있으면 표시.
- **Google** — Google로 한 번이라도 로그인한 적이 있으면 표시.

![프로필 페이지 — 연결된 계정](./img/auth-profile.png)

각 행에는 **Unlink** 버튼이 있습니다. 포털은 본인이 로그인 수단을 모두 잃는 상황을 방지합니다(마지막 인증 수단 보호).
