...
전환은 행위자, 이전 상태, 새 상태, 필수 사유 메시지와 함께 감사 로그에 기록됩니다.
...
- **상태** — 현재 VEX 상태.
- **발견 시각** — 결과가 처음 등장한 시점.
- **마지막 확인** — 결과가 마지막으로 확인된 스캔 시점.

상단 인라인 필터 바: 심각도, 상태, 컴포넌트, 발견 범위.

## 드로어 — 결과 상세

행을 클릭하면 다음을 봅니다.

- **CVE 요약** — 제목, 설명, CWE, CVSS 벡터.
- **영향 받는 버전** — 상위에서 보고한 영향 범위와 본 프로젝트 컴포넌트 버전 강조.
- **참고 자료** — 벤더 권고, 수정 커밋, 익스플로잇 데이터베이스.
- **수정 가용성** — 상위 수정 존재 여부와 수정이 포함된 버전.
- **프로젝트 이력** — 결과가 등장한 모든 스캔과 타임스탬프.
- **분류** — VEX 상태 드롭다운, 사유 박스, **Save**. `developer` 이상만.
...
대시보드의 **CVE 재탐지** 배너는 가장 최근 동기화 실행을 요약합니다: 처리된 피드 수, 신규 결과 수, 실행 타임스탬프.
...
1. 상태 배지가 테이블에서 즉시 갱신.
2. 감사 로그에 `vuln_finding.update`가 `previous_state`, `new_state`, `justification`과 함께 기록.
