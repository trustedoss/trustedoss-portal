...
2. 포털이 시크릿을 생성해 1회만 표시합니다. 복사해 두세요.
3. 전송 URL을 메모 — `https://trustedoss.example.com/api/v1/webhooks/github`.
...
2. 포털이 시크릿 토큰을 생성해 1회만 표시합니다. 복사해 두세요.
3. 전송 URL을 메모 — `https://trustedoss.example.com/api/v1/webhooks/gitlab`.
...
- 프로젝트의 `auto_scan_on_webhook=false`. **Project Settings → CI/CD**에서 켜기.
