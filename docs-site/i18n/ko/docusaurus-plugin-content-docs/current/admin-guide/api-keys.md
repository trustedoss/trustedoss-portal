...
- **`org`** — 조직 전체에서 행위하며, 발급자가 호출할 수 있던 어떤 endpoint 든 호출 가능.
- **`team`** — 특정 팀을 대신해 행위하며, 팀 간 호출은 403으로 실패.
- **`project`** — 특정 프로젝트에 묶이며, 해당 프로젝트 외의 호출은 403으로 실패.
...
- `api_key.revoked` — 명시적 폐기 시 API Key 서비스가 발신하는 **structlog 이벤트 전용**(actor, target prefix). v2.0.0 에서는 `audit_logs` 행을 **생성하지 않습니다**. 단, ORM 리스너가 `revoked_at` 변경에 대한 `api_keys.update` 행을 여전히 기록하므로 폐기 자체는 감사 테이블에 캡처됩니다 — 다만 structlog 이벤트 이름이 아닌 `target_table=api_keys&action=update` 로 기록됩니다.
...
- 포털은 두 실패 모드를 구분합니다:
  ...
  - **403** = 자격증명은 유효하지만 Key 의 scope 가 리소스를 커버하지
    않음(예: `team`-scope Key 가 `org` 전용 엔드포인트 호출).
