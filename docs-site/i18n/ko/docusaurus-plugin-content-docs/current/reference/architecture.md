...
- **Access token** — JWT, 30분 수명, `RS256` 서명, 인앱 메모리 전용.
- **Refresh token** — 7일 수명, **회전 + 재사용 탐지**. HttpOnly + Secure + SameSite=Lax 쿠키.
- **API Key** — `tos_<prefix>_<secret>`. bcrypt 해시 — 전체 Key는 생성 시 1회 표시.
- **CSRF** — `/api/v1/auth/*`로의 쿠키 인증 POST에 토큰. Bearer 토큰은 구조상 CSRF 면역.
- **레이트 리밋** — 로그인은 IP 키 5/분, 429 + `Retry-After`. 그 외 대부분 엔드포인트는 사용자 키.
...
- **로그** — `docker-compose logs <service>`(구조화된 JSON).
- **Health** — `/health`(backend), `/healthz`(frontend), `/admin/health` UI.
- **Metrics** — `/metrics`(backend, Prometheus exposition format) — Phase 6.

선택적 Jaeger 오버레이(`docker-compose.tracing.yml`)는 OpenTelemetry SDK + OTLP exporter를 Jaeger all-in-one에 연결합니다. 샘플링은 **기본 꺼짐** — 사고 조사 시에만 `OTEL_SAMPLER=parentbased_always_on`으로 켜세요.
